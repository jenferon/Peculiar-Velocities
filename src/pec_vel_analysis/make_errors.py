...
for nn in range(0,len(names)):
    print("opening: "+base+"normalised_EoR_1000hrs/"+names[nn])
    hdulist = fits.open(base+"normalised_EoR_1000hrs/"+names[nn])
    NOISE = hdulist[0].data
    print("opened: "+base+"normalised_EoR_1000hrs/"+names[nn])
...
np.savetxt(base+"errors_on_FastICA_EoR_1Dps.txt",errors)
...
np.savetxt(base+"errors_on_FastICA_EoR_mups.txt",errors_mu.reshape((mubins*kbins*3)))
