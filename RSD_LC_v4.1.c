...
int main(int argc, char *argv[])
{
    ...
    if (!(count_zbox = (int *)malloc(LC_size * sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    if (!(count_nu_slice = (int *)malloc(LC_size * sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    ...
    if (lc_on == 0)
    {
        for (int mm = 0; mm < n_maps; mm++)
        {
            ...
            for (int ind = 0; ind < Dim * Dim; ind++)
            {
                ...
                count_zbox[mm + n_maps * ind] = 0;
                count_nu_slice[mm + n_maps * ind] = 0;
            }
        }
    }
    else
    ...
    if (!(all_count_zbox  = (int *)malloc(LC_size * sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    if (!(all_count_nu_slice  = (int *)malloc(LC_size * sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    ...
    int kk = 0;
    if (myid == 0)
        printf("Filling maps from redshift %f \n", zbox);
    for (z = zmax; z >= zmin; kk++)
    {
        ...
        for (int mm = 0; mm < n_maps; mm++)
        {
            ...
            if ((lc_on == 1 && z_p[mm] < (zbox + dz) && z_p[mm] >= zbox && (k_sm == k_sm_p)) && mm_fill[mm] > fabs(r - r_p) || (lc_on == 0)  && (mm_fill[mm] > fabs(r - r_p))) // add mmfill into here
            {
                //printf("mm fill: %f \n", mm_fill[mm]);
                            ...
                            { // added 19/7
                                ...
                                if (lc_on == 1 || (lc_on == 0 && !(((nu_i < (nu21 - del_nu_21 / 2.0)) && (nu_f < (nu21 - del_nu_21 / 2.0))) || ((nu_i > (nu21 + del_nu_21 / 2.0)) && (nu_f > (nu21 + del_nu_21 / 2.0)))))) // added 10/2 to charcterise in terms of non-events
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        k_p = round(k_r_p/del_r);
                                        k_sm_p = k_p;// pixel index equivalent in small box boundary conditions 
                                        if (k_sm_p !=  k_sm){count_nu_slice[mm + n_maps * (jj + Dim * ii)] += 1;}
                                    ...
                                    if (z_p[mm] > (zbox + dz) && z_p[mm] < zbox && lc_on == 0){count_zbox[mm + n_maps * (jj + Dim * ii)] += 1;}
    ...
    MPI_Allreduce(count_zbox, all_count_zbox, LC_size, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(count_nu_slice, all_count_nu_slice, LC_size, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    ...
    if (myid == 0)
    {
        sprintf(fname, "%s/deltaTb_RSD/LightconeRSD_N%d_FOV%06.4f_dnu%04.2fMHz_%06.2fMHz_%06.2fMHz_ds%08.6f_div%05.2f_pv%d_oneevent%d_evo%d_lcon%d_dz_%d.dat", argv[1], Dim, FoVdeg, del_nu_lc, nu1, nu2, del_s, dsdiv, pv, oneevent, evo, lc_on, dz);
    ...
    if (myid == 0)
    {
        sprintf(fname, "%s/deltaTb_RSD/Oneevent_N%d_FOV%06.4f_dnu%04.2fMHz_%06.2fMHz_%06.2fMHz_ds%08.6f_div%05.2f_pv%d_oneevent%d_evo%d_lcon%d_dz_%d.dat", argv[1], Dim, FoVdeg, del_nu_lc, nu1, nu2, del_s, dsdiv, pv, oneevent, evo, lc_on, dz);
        if ((fid = fopen(fname, "wb")) == NULL)
        {
            printf("Cannot open file:%s...\n", fname);
            exit(1);
    ...
    if (myid == 0)
    {
        sprintf(fname, "%s/deltaTb_RSD/nuslicecount-N%d_FOV%06.4f_dnu%04.2fMHz_%06.2fMHz_%06.2fMHz_ds%08.6f_div%05.2f_pv%d_oneevent%d_evo%d_lcon%d_dz_%d.dat", argv[1], Dim, FoVdeg, del_nu_lc, nu1, nu2, del_s, dsdiv, pv, oneevent, evo, lc_on, dz);
        if ((fid = fopen(fname, "wb")) == NULL)
        {
            printf("Cannot open file:%s...\n", fname);
            exit(1);
        }
        fwrite(all_count_nu_slice, sizeof(int), LC_size, fid);
        fclose(fid);
    }

    if (myid == 0)
    {
        sprintf(fname, "%s/deltaTb_RSD/zboxcount_N%d_FOV%06.4f_dnu%04.2fMHz_%06.2fMHz_%06.2fMHz_ds%08.6f_div%05.2f_pv%d_oneevent%d_evo%d_lcon%d_dz_%d.dat", argv[1], Dim, FoVdeg, del_nu_lc, nu1, nu2, del_s, dsdiv, pv, oneevent, evo, lc_on, dz);
        if ((fid = fopen(fname, "wb")) == NULL)
        {
            printf("Cannot open file:%s...\n", fname);
            exit(1);
        }
        fwrite(all_count_zbox, sizeof(int), LC_size, fid);
        fclose(fid);
    }
