...
int main(int argc, char *argv[])
{
    ...
    oneevent = 0; // set to 1 if want intital intensity of cell always to be CMB
    dnu_off = 0;  // set to 1 if want to set d_nu = 0
    evo = 1;      // set to 1 to interpolate between z, z+dz boxes according to z_cell
    lc_on = 1;    // output original method lightcone if set to 1
    //******************************************************************************************
    ...
    float *nu_first;
    long long LC_size;
    long long Z_box_count_size;
    long long nu_count_size;
    double fraction;
    double Tcmbdz;
    float ave = 0.0;
    long long aven = 0;
    ...
    if (global_use_Lya_xrays == 0)
        printf("Lya and xray use set to false - assuming TS>>TCMB in t21 calculation.\n");
    if (argc > 7)
    {
        zmin = atof(argv[7]);
        //printf("global zmin f")
        if (zmin < global_Zminsim)
            printf("using global Zmin");
            zmin = global_Zminsim;
        if (argc > 8)
        {
            zmax = atof(argv[8]);
            if (zmax > global_Zmaxsim)
                zmax = global_Zmaxsim;
            if (argc == 10)
                dz = atof(argv[9]);
            else
    ...
    n_boxes = floor((zmax - zmin) / dz) + 1;
    if (myid == 0)
        printf("This code will produce %d maps spaced equally by %f MHz\n", n_maps, del_nu_lc);
        printf("From %d boxes spaced between z = %f and %f \n", n_boxes, zmax, zmin);
    del_omega = FoV / (Dim);
    LC_size = n_maps * Dim * Dim;
    Z_box_count_size = n_maps*n_boxes;
    nu_count_size = n_maps*(n_maps+10);
    if (!(mm_fill = (float *)malloc(LC_size * sizeof(float))))
    {
    ...
    if (!(count = (int *)malloc(LC_size * sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    if (!(Z_nu_array = (int *)malloc(Z_box_count_size*sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    if (!(count_nu_slice = (int *)malloc(nu_count_size * sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    ...
    for (int mm = 0; mm < n_maps; mm++)
    {
        for (int nn = 0; nn < n_boxes; nn++)
        {
            Z_nu_array[mm + n_maps * nn]=0;
        }
    }
    for (int mm = 0; mm < n_maps; mm++)
    {
        for (int nn = 0; nn < (n_maps+10); nn++)
        {
            count_nu_slice[mm + n_maps * nn] = 0;
        }
    }
    ...
    if (!(all_count = (int *)malloc(LC_size * sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    if (!(all_count_nu_slice  = (int *)malloc(nu_count_size * sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    if (!(all_Z_nu_array = (int *)malloc(Z_box_count_size*sizeof(int))))
    {
        printf("Problem allocating memory to arrays...\n");
        exit(1);
    }
    ...
    if (del_s_pix / 5.0 < del_s)
        //printf("making approximation that ds = ds_pix/5 \n");
        del_s = del_s_pix / 5.0; // so always one integration point per pixel

    if (lc_on == 1)
        del_s = del_s_pix / 2.0; // if exactly cell size numerical error sbuild up and miss slices.
    if (myid == 0)
        printf("This code will step with ds %f \n", del_s);
    ...
    sprintf(fname, "%s/Velocity/vel_z%.3f_N%ld_L%.1f_3.dat", argv[1], zmax - dz, global_N_smooth, (global_L / global_hubble));
    fid = fopen(fname, "rb");
    if (fid == NULL)
    {
        printf("Error reading vel file... Check path or if the file exists...");
        printf("Path: %s", fname);
        exit(1);
    }
    ...
    for (i = 0; i < global_N3_smooth; i++)
        v_c[i] = temp[i]; // velocity in proper units divided by c

    sprintf(fname, "%s/Velocity/vel_z%.3f_N%ld_L%.1f_3.dat", argv[1], zmax, global_N_smooth, (global_L / global_hubble));
    fid = fopen(fname, "rb");
    if (fid == NULL)
    {
        printf("Error reading vel file... Check path or if the file exists...");
    ...
    fclose(fid);
    for (i = 0; i < global_N3_smooth; i++)
        v_cdz[i] = temp[i]; // velocity in proper units divided by c

    /************** READ IN VELOCITY AND VELOCITY GRADIENT BOXES ***************/
    sprintf(fname, "%s/Velocity/dvdr_z%.3f_N%ld_L%.1f_3.dat", argv[1], zmax - dz, global_N_smooth, (global_L / global_hubble));
    fid = fopen(fname, "rb");
    if (fid == NULL)
    {
        printf("Error reading dvdr file... Check path or if the file exists...");
        printf("Path: %s \n", fname);
        exit(1);
    }
    fread(temp, sizeof(float), global_N3_smooth, fid);
    fclose(fid);
    for (i = 0; i < global_N3_smooth; i++)
        dvds_H[i] = temp[i]; // dvds divided by H

    sprintf(fname, "%s/Velocity/dvdr_z%.3f_N%ld_L%.1f_3.dat", argv[1], zmax, global_N_smooth, (global_L / global_hubble));
    fid = fopen(fname, "rb");
    if (fid == NULL)
    {
        printf("Error reading dvdr file... Check path or if the file exists...");
    ...
    fclose(fid);
    for (i = 0; i < global_N3_smooth; i++)
        dvds_Hdz[i] = temp[i]; // dvds divided by H

    /************ READ IN DENSITY AND IONIZATION FIELDS TO CALC nHI  ***********/
    sprintf(fname, "%s/delta/deltanl_z%.3f_N%ld_L%.1f.dat", argv[1], zmax - dz, global_N_smooth, (global_L / global_hubble));
    fid = fopen(fname, "rb");
    if (fid == NULL)
    {
    ...
    sprintf(fname, "%s/delta/deltanl_z%.3f_N%ld_L%.1f.dat", argv[1], zmax, global_N_smooth, (global_L / global_hubble));
    fid = fopen(fname, "rb");
    if (fid == NULL)
    {
        printf("Error opening file:%s\n", fname);
    ...
    sprintf(fname, "%s/Ionization/xHII_z%.3f_N%ld_L%.1f.dat", argv[1], zmax - dz, global_N_smooth, (global_L / global_hubble));
    ...
    for (i = 0; i < global_N3_smooth; i++)
        nHI[i] = (1. - xHII[i]) * (1. + dnl[i]) * global_omega_b * 3.0 * pow(H0 * global_hubble, 2.0) / (8.0 * PI * G) * pow(1.0 + zmax - dz, 3.0) * 0.76 / mbar;

    sprintf(fname, "%s/Ionization/xHII_z%.3f_N%ld_L%.1f.dat", argv[1], zmax, global_N_smooth, (global_L / global_hubble));
    if ((fid = fopen(fname, "rb")) == NULL)
    {
        printf("Error opening file:%s\n", fname);
        exit(1);
    ...
    for (i = 0; i < global_N3_smooth; i++)
        nHIdz[i] = (1. - xHIIdz[i]) * (1. + dnldz[i]) * global_omega_b * 3.0 * pow(H0 * global_hubble, 2.0) / (8.0 * PI * G) * pow(1.0 + zmax, 3.0) * 0.76 / mbar;
    /************** READ IN XRAY FILES IF USING TS ****************************/
    if (zmax - dz > (global_Zminsfr - global_Dzsim / 10) && global_use_Lya_xrays == 1)
    {
        Tcmb = Tcmb0 * (1. + zmax - dz);
        Tcmbdz = Tcmb0 * (1. + zmax);
        sprintf(fname, "%s/x_c/xc_z%.3lf_N%ld_L%.1f.dat", argv[1], zmax - dz, global_N_smooth, (global_L / global_hubble));
        if ((fid = fopen(fname, "rb")) == NULL)
        {
            printf("Error opening file:%s\n", fname);
            exit(1);
        ...
        sprintf(fname, "%s/x_c/xc_z%.3lf_N%ld_L%.1f.dat", argv[1], zmax, global_N_smooth, (global_L / global_hubble));
        if ((fid = fopen(fname, "rb")) == NULL)
        {
            printf("Error opening file:%s\n", fname);
            exit(1);
        ...
        fclose(fid);
        for (i = 0; i < global_N3_smooth; i++)
            t21dz[i] = (double)temp[i];

        sprintf(fname, "%s/Lya/xalpha_z%.3lf_N%ld_L%.1f.dat", argv[1], zmax - dz, global_N_smooth, (global_L / global_hubble));
        if ((fid = fopen(fname, "rb")) == NULL)
        {
            printf("Error opening file:%s\n", fname);
            exit(1);
        ...
        sprintf(fname, "%s/Lya/xalpha_z%.3lf_N%ld_L%.1f.dat", argv[1], zmax, global_N_smooth, (global_L / global_hubble));
        if ((fid = fopen(fname, "rb")) == NULL)
        {
            printf("Error opening file:%s\n", fname);
            exit(1);
        ...
        sprintf(fname, "%s/xrays/TempX_z%.3lf_N%ld_L%.1f.dat", argv[1], zmax - dz, global_N_smooth, (global_L / global_hubble));
        if ((fid = fopen(fname, "rb")) == NULL)
        {
            printf("Error opening file:%s\n", fname);
            exit(1);
        ...
        sprintf(fname, "%s/xrays/TempX_z%.3lf_N%ld_L%.1f.dat", argv[1], zmax, global_N_smooth, (global_L / global_hubble));
        if ((fid = fopen(fname, "rb")) == NULL)
        {
            printf("Error opening file:%s\n", fname);
            exit(1);
    ...
    else
    {
        ...
        for (i = 0; i < global_N3_smooth; i++)
            TSdz[i] = 100000.0;
        for (i = 0; i < global_N3_smooth; i++)
            t21[i] = 1.0; // added 13/09/17
        for (i = 0; i < global_N3_smooth; i++)
            t21dz[i] = 1.0; // added 13/09/17
    }
    ...
    int kk = 0;
    if (myid == 0)
        printf("Filling maps from redshift %.3f \n", zbox);
    for (z = zmax; z >= zmin; kk++)
    {
        ...
        if (z < zbox)
        {
            ...
            if (del_s_pix / 5.0 < del_s)
                del_s = del_s_pix / 5.0; // so good sampling per pixel
            if (lc_on == 1)
                del_s = del_s_pix / 2.0;
            if (myid == 0)
                printf("This code will step with ds %f \n", del_s);
            ...
            /* upload boxes */
            /************** READ IN VELOCITY AND VELOCITY GRADIENT BOXES ***************/
            sprintf(fname, "%s/Velocity/dvdr_z%.3f_N%ld_L%.1f_3.dat", argv[1], zbox, global_N_smooth, (global_L / global_hubble));
            fid = fopen(fname, "rb");
            if (fid == NULL)
            {
                printf("Error reading dvdr file... Check path or if the file exists...");
            ...
            sprintf(fname, "%s/Velocity/dvdr_z%.3f_N%ld_L%.1f_3.dat", argv[1], zbox + dz, global_N_smooth, (global_L / global_hubble));
            fid = fopen(fname, "rb");
            if (fid == NULL)
            {
                printf("Error reading dvdr file... Check path or if the file exists...");
            ...
            /************ READ IN DENSITY AND IONIZATION FIELDS TO CALC nHI  ***********/
            sprintf(fname, "%s/delta/deltanl_z%.3f_N%ld_L%.1f.dat", argv[1], zbox, global_N_smooth, (global_L / global_hubble));
            fid = fopen(fname, "rb");
            if (fid == NULL)
            {
                printf("Error opening file:%s\n", fname);
            ...
            sprintf(fname, "%s/delta/deltanl_z%.3f_N%ld_L%.1f.dat", argv[1], zbox + dz, global_N_smooth, (global_L / global_hubble));
            fid = fopen(fname, "rb");
            if (fid == NULL)
            {
                printf("Error opening file:%s\n", fname);
            ...
            sprintf(fname, "%s/Ionization/xHII_z%.3f_N%ld_L%.1f.dat", argv[1], zbox, global_N_smooth, (global_L / global_hubble));
            if ((fid = fopen(fname, "rb")) == NULL)
            {
                printf("Error opening file:%s\n", fname);
                exit(1);
            ...
            for (i = 0; i < global_N3_smooth; i++)
                nHI[i] = (1.0 - xHII[i]) * (1. + dnl[i]) * global_omega_b * 3.0 * pow(H0 * global_hubble, 2.0) / (8.0 * PI * G) * pow(1.0 + zbox, 3.0) * 0.76 / mbar;

            sprintf(fname, "%s/Ionization/xHII_z%.3f_N%ld_L%.1f.dat", argv[1], zbox + dz, global_N_smooth, (global_L / global_hubble));
            if ((fid = fopen(fname, "rb")) == NULL)
            {
                printf("Error opening file:%s\n", fname);
                exit(1);
            ...
            for (i = 0; i < global_N3_smooth; i++)
                xHIIdz[i] = (temp[i]);
            for (i = 0; i < global_N3_smooth; i++)
                nHIdz[i] = (1. - xHIIdz[i]) * (1. + dnldz[i]) * global_omega_b * 3.0 * pow(H0 * global_hubble, 2.0) / (8.0 * PI * G) * pow(1.0 + zbox + dz, 3.0) * 0.76 / mbar;
            /************** READ IN XRAY FILES IF USING TS ****************************/
            ...
            if (zbox > (global_Zminsfr - global_Dzsim / 10) && global_use_Lya_xrays == 1)
            {
                ...
                sprintf(fname, "%s/x_c/xc_z%.3lf_N%ld_L%.1f.dat", argv[1], zbox, global_N_smooth, (global_L / global_hubble));
                if ((fid = fopen(fname, "rb")) == NULL)
                {
                    printf("Error opening file:%s\n", fname);
                    exit(1);
                ...
                sprintf(fname, "%s/x_c/xc_z%.3lf_N%ld_L%.1f.dat", argv[1], zbox + dz, global_N_smooth, (global_L / global_hubble));
                if ((fid = fopen(fname, "rb")) == NULL)
                {
                    printf("Error opening file:%s\n", fname);
                    exit(1);
                ...
                sprintf(fname, "%s/Lya/xalpha_z%.3lf_N%ld_L%.1f.dat", argv[1], zbox, global_N_smooth, (global_L / global_hubble));
                if ((fid = fopen(fname, "rb")) == NULL)
                {
                    printf("Error opening file:%s\n", fname);
                    exit(1);
                ...
                sprintf(fname, "%s/Lya/xalpha_z%.3lf_N%ld_L%.1f.dat", argv[1], zbox + dz, global_N_smooth, (global_L / global_hubble));
                if ((fid = fopen(fname, "rb")) == NULL)
                {
                    printf("Error opening file:%s\n", fname);
                    exit(1);
                ...
                sprintf(fname, "%s/xrays/TempX_z%.3lf_N%ld_L%.1f.dat", argv[1], zbox, global_N_smooth, (global_L / global_hubble));
                if ((fid = fopen(fname, "rb")) == NULL)
                {
                    printf("Error opening file:%s\n", fname);
                    exit(1);
                ...
                sprintf(fname, "%s/xrays/TempX_z%.3lf_N%ld_L%.1f.dat", argv[1], zbox + dz, global_N_smooth, (global_L / global_hubble));
                if ((fid = fopen(fname, "rb")) == NULL)
                {
                    printf("Error opening file:%s\n", fname);
                    exit(1);
        ...
        for (int jjj = 0;; jjj++)
            if (k_sm < 0)
                k_sm += global_N_smooth;
            else
                break;
        for (int jjj = 0;; jjj++)
            if (k_sm >= global_N_smooth)
                k_sm -= global_N_smooth;
            else
                break;
        ...
        {
            nu_p = nu2 + (2.0 * mm + 1.0) * del_nu_lc / 2.0;
            if (nu_p < nu21 / (z + 1) - 2 * del_nu_lc)
            {
                mm_min = mm;
            } // Only searches maps within small range of frequencies. Makes code much faster.
            if (nu_p < nu21 / (z + 1.) + 2 * del_nu_lc)
            {
                mm_max = mm;
            }
        ...
        for (int mm = 0; mm < n_maps; mm++)
        {
            ...
            if (lc_on == 1)
            {
                ...
                for (int jjj = 0;; jjj++)
                    if (k_sm_p < 0)
                        k_sm_p += global_N_smooth;
                    else
                        break;
                for (int jjj = 0;; jjj++)
                    if (k_sm_p >= global_N_smooth)
                        k_sm_p -= global_N_smooth;
                    else
                        break;
            }
            //printf("mm: %d, r-rp %f  , mmfill %d \n", mm,fabs(r - r_p), mm_fill[mm]);
            if ((lc_on == 1 && z_p[mm] < (zbox + dz) && z_p[mm] >= zbox && (k_sm == k_sm_p) && mm_fill[mm] > fabs(r - r_p)) || (lc_on == 0)) // add mmfill into here
            {
                //re zero array if already filled slice and re filling with closer slice
                if(lc_on == 1)
                {
                    //printf("refilling slice \n");
                    int temp = (int)((zbox-zmin)/dz);
                    Z_nu_array[mm + n_maps * temp]=0;
                    float nu_temp = nu21/(1+z); //seems to be off by a factor of 10...
                    int xxtemp = abs((nu2-5*del_nu_lc) - nu_temp)/del_nu_lc;
                    count_nu_slice[mm + n_maps * xxtemp]=0;
                    mm_fill[mm] = fabs(r - r_p);
                ...
                for (int ii = 0; ii < Dim; ii++)
                {
                    for (int jj = 0; jj < Dim; jj++)
                    {
                        ...
                        if (ij % numProcs == myid)
                        {
                            ...
                            for (int jjj = 0;; jjj++)
                                if (j_sm < 0)
                                    j_sm += global_N_smooth;
                                else
                                    break;
                            for (int jjj = 0;; jjj++)
                                if (j_sm >= global_N_smooth)
                                    j_sm -= global_N_smooth;
                                else
                                    break;
                            ...
                            for (int jjj = 0;; jjj++)
                                if (i_sm < 0)
                                    i_sm += global_N_smooth;
                                else
                                    break;
                            for (int jjj = 0;; jjj++)
                                if (i_sm >= global_N_smooth)
                                    i_sm -= global_N_smooth;
                                else
                                    break;
                            ...
                            { // added 19/7
                                ...
                                if (lc_on == 1 || (lc_on == 0 && !(((nu_i < (nu21 - del_nu_21 / 2.0)) && (nu_f < (nu21 - del_nu_21 / 2.0))) || ((nu_i > (nu21 + del_nu_21 / 2.0)) && (nu_f > (nu21 + del_nu_21 / 2.0)))))) // added 10/2 to charcterise in terms of non-events
                                {
                                    ...
                                    if (lc_on == 1)
                                    {
                                        ...
                                        if (dvds_H_pix > maxcut)
                                        {
                                            dvds_H_pix = maxcut;
                                            nmaxcut++;
                                        }
                                        else if (dvds_H_pix < mincut)
                                        {
                                            dvds_H_pix = mincut;
                                            nmincut++;
                                        }

                                        I21[mm + n_maps * (jj + Dim * ii)] = 23.0 / 1000. * (1. + dnl_pix) * t21_pix / (1. + 1. * dvds_H_pix) * (0.7 / global_hubble) * (global_omega_b * global_hubble * global_hubble / 0.02) * sqrt((0.15 / global_omega_m / global_hubble / global_hubble) * (1. + zbox) / 10.) * (1. - xHII_pix);
                                        del_I = 0.0;
                                    }
                                    ...
                                    //add counts once 21cm event has also been added 
                                    int temp = (int)((zbox-zmin)/dz);
                                    Z_nu_array[mm + n_maps * temp] += 1;
                                    float nu_temp = nu21/(1+z); //seems to be off by a factor of 10...
                                    int xxtemp = abs((nu2-5*del_nu_lc) - nu_temp)/del_nu_lc;
                                    //printf("r: %f, zp: %f, z: %f, nu_exp: %f, nu_p: %f, xx: %d, yy: %d\n",r, z_p[mm], ztemp, nu_temp, nu_p, xxtemp, yytemp);
                                    count_nu_slice[mm + n_maps * xxtemp] += 1;
                                    count[mm + n_maps * (jj + Dim * ii)] += 1;
                                    //if (ii ==  291 && jj == 407 && mm == 310){printf("nu_i: %f, nu_f %f, k_r: %f \n", nu_i, nu_f, k_r);}//los pixel 
                                }
    ...
    MPI_Allreduce(count, all_count, LC_size, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(Z_nu_array, all_Z_nu_array, Z_box_count_size, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(count_nu_slice, all_count_nu_slice, nu_count_size, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
        ...
        for (int mm = 0; mm < n_maps; mm++)
        {
            ...
            for (int ii = 0; ii < Dim; ii++)
            {
                for (int jj = 0; jj < Dim; jj++)
                {
                    ...
                    ave = ave + all_I21[mm + n_maps * (jj + Dim * ii)];
                    aven = aven + 1;
                }
            }
        }
    ...
    ave = ave / aven * 1000.;
    if (myid == 0)
        printf("average pixel value (mK): %e \n", ave);

    // OUTPUT ALL MAPS WITH DEL_NU SEPARATION

    if (myid == 0)
    {
        sprintf(fname, "%s/deltaTb_RSD/LightconeRSD_N%d_FOV%06.4f_dnu%04.2fMHz_%06.2fMHz_%06.2fMHz_ds%08.6f_div%05.2f_pv%d_oneevent%d_evo%d_lcon%d_dz_%06.2f.dat", argv[1], Dim, FoVdeg, del_nu_lc, nu1, nu2, del_s, dsdiv, pv, oneevent, evo, lc_on, dz);
    ...
    if (myid == 0)
    {
        sprintf(fname, "%s/deltaTb_RSD/Oneevent_N%d_FOV%06.4f_dnu%04.2fMHz_%06.2fMHz_%06.2fMHz_ds%08.6f_div%05.2f_pv%d_oneevent%d_evo%d_lcon%d_dz_%06.2f.dat", argv[1], Dim, FoVdeg, del_nu_lc, nu1, nu2, del_s, dsdiv, pv, oneevent, evo, lc_on, dz);
        if ((fid = fopen(fname, "wb")) == NULL)
        {
            printf("Cannot open file:%s...\n", fname);
            exit(1);
        }
        fwrite(all_count, sizeof(int), LC_size, fid);
        fclose(fid);
    }

    if (myid == 0)
    {
        sprintf(fname, "%s/deltaTb_RSD/nuslicecount_N%d_FOV%06.4f_dnu%04.2fMHz_%06.2fMHz_%06.2fMHz_ds%08.6f_div%05.2f_pv%d_oneevent%d_evo%d_lcon%d_dz_%06.2f.dat", argv[1], Dim, FoVdeg, del_nu_lc, nu1, nu2, del_s, dsdiv, pv, oneevent, evo, lc_on, dz);
        if ((fid = fopen(fname, "wb")) == NULL)
       {
            printf("Cannot open file:%s...\n", fname);
            exit(1);
        }
        fwrite(all_count_nu_slice, sizeof(int), nu_count_size, fid);
        fclose(fid);
    }

    if (myid == 0)
    {
        sprintf(fname, "%s/deltaTb_RSD/z_nu_array_N%d_FOV%06.4f_dnu%04.2fMHz_%06.2fMHz_%06.2fMHz_ds%08.6f_div%05.2f_pv%d_oneevent%d_evo%d_lcon%d_dz_%06.2f.dat", argv[1], Dim, FoVdeg, del_nu_lc, nu1, nu2, del_s, dsdiv, pv, oneevent, evo, lc_on, dz);
        if ((fid = fopen(fname, "wb")) == NULL)
        {
            printf("Cannot open file:%s...\n", fname);
            exit(1);
        }
        fwrite(all_Z_nu_array, sizeof(int), Z_box_count_size, fid);
        fclose(fid);
    }
...
float cell_interp(float *box, float del_r, float fraction_i, float fraction_j, int i_sm, int j_sm, int k_sm)
{
    ...
    {
        if (j_sm == 0)
            j2 = global_N_smooth - 1;
        else
            j2 = j_sm - 1;
    }
    else
    {
        if (j_sm == global_N_smooth - 1)
            j2 = 0;
        else
            j2 = j_sm + 1;
    }
    ...
    {
        if (i_sm == 0)
            i2 = global_N_smooth - 1;
        else
            i2 = i_sm - 1;
    }
    else
    {
        if (i_sm == global_N_smooth - 1)
            i2 = 0;
        else
            i2 = i_sm + 1;
    }

    val = cell_c * box[(k_sm + global_N_smooth * (j_sm + global_N_smooth * i_sm))] + cell_x * box[(k_sm + global_N_smooth * (j_sm + global_N_smooth * i2))] + cell_y * box[(k_sm + global_N_smooth * (j2 + global_N_smooth * i_sm))] + cell_xy * box[(k_sm + global_N_smooth * (j2 + global_N_smooth * i2))];
